...
Using the Linear connector, list issues on team `ENG` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `ENG-NNN-short-slug`, using the issue's real identifier.
...
**This repository (static Cloudflare Pages site, no build step):** after any edit
to `index.html`, `assets/css/*.css`, or `assets/js/*.js`, run
`python3 scripts/check-cache-bust.py --fix` (re-stamps asset `?v=` content
hashes) then `python3 scripts/prerender.py` (regenerates the per-route HTML),
and commit the regenerated files. The authoritative gate is
`bash scripts/pre-push-check.sh` — trademark scan, prerender drift, route/link
parity, and cache-bust — which must exit 0; the same checks run in CI on the PR.
Never introduce ServiceNow "partner/authorised/certified" language (the
trademark check forbids it); Halo partner language is allowed. See `CLAUDE.md`.
...
- `Closes ENG-NNN`, using the real Linear issue identifier
