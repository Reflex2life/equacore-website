...
## Part 2 — built + verified (2026-07-11)

- **Design refinement during implementation:** the existing `auto` classification in Match & Classify conflated two different signals — genuine delivery failures (mailer-daemon/postmaster/undeliverable) and out-of-office/vacation auto-replies (a live human, just away). Treating both as "bounce → reject" would have wrongly killed the cadence for anyone on vacation. Split into a new `bounce` route (genuine NDR signals only) that triggers the reject flow, while OOO/vacation auto-replies keep their existing behavior (dropped, unchanged, cadence continues since the mailbox is not actually dead).
- **Recipient-address fix:** a bounce notification's `From` is the mail server (e.g. `mailer-daemon@zoho.com`), not the original prospect — matching the existing `Get Reply Row` lookup (keyed on sender email) would never find the right row. Added extraction of the actual bounced recipient address from the NDR body (excluding the mailer-daemon address itself and our own sending domains) into a new `bouncedEmail` field, and a dedicated `Get Bounced Row` lookup keyed on that instead.
- **Built:** `Match & Classify` code updated (adds `isBounce`/`bouncedEmail`), `Route Reply` switch gained a third `bounce` rule, four new nodes added (`Get Bounced Row` → `Mark Bounced` → `Card to Rejected (Bounce)` → `Bounce Comment`), wired from the switch's previously-dead third output.
- **Verified (execution 2318):** `test_workflow` with a synthetic NDR pinned at the IMAP trigger confirmed `Match & Classify` correctly produced `route: "bounce"`, `bouncedEmail: "test-bounce-verify@example-inactive.com"` — correctly excluding the mailer-daemon sender address and extracting the real failed recipient from the body. Downstream nodes are credentialed (ClickUp/Data Table) so `test_workflow` pins/simulates them rather than executing for real; they are structural copies of the already-proven `Mark Unsubscribed → Card to Rejected` pattern live in this same workflow, so confidence there rests on that reuse rather than a live end-to-end run.
- **Activated:** unpublished then republished (per the known IMAP-stall gotcha for this workflow) to force a fresh IDLE reconnect. Confirmed active afterward.
